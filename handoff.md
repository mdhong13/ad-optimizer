...
- **🚨 인프라 자격증명 로테이션 점검 — MongoDB IP 변경 + Railway env 재동기**
  ...
  - ✅ **Naver 해결**: NAVER_CLIENT_ID 앱이 401(Naver측 자동무효, 값 갱신해도 復活 안 됨). knowin_crawler 를 `NAVER_OPENAPI_CLIENT_ID/SECRET` 우선(fallback NAVER_CLIENT_ID)으로 repoint → 검색 3건 정상. Railway 에 NAVER_OPENAPI_* push. 옛 NAVER_CLIENT_ID 앱 살리려면 Naver 개발자콘솔(앱 측) 일 — 불필요.
