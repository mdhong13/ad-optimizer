...
8. **메타 발화 금지** — "저희가 보유한 자료", "참고 자료에 따르면", "저희 데이터에는" 같이 답변자를 "자료 보유자" 로 보이게 하는 표현 금지. 본인이 직접 아는 것처럼 자연스럽게 작성.
...
def _check_guardrails(body: str) -> list[str]:
    ...
    # 메타 발화 — 답변자가 "자료 보유자" 로 보이게 만드는 표현 (페르소나 깨짐)
    meta_phrases = [
        "저희가 보유한 자료",
        "저희 자료",
        "참고 자료에 따르면",
    ...
    for mp in meta_phrases:
        if mp in body:
            warnings.append(f"메타 발화 포함: '{mp}' (답변자 페르소나 깨짐 — 자연스러운 톤으로 재작성 권장)")
